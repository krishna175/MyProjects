...
def homepage():
    ...
    home = Tk()
    mixer.init()
    mixer.music.load('Images/game background.mp3')
    mixer.music.play()
    home.configure(bg="white")
    home.title('DODGE THE BALL 2.0')
    home.iconbitmap("Images/dodgeball_icon.ico")
    ...
    #backgound image
    temp_size = Image.open("Images/backgound.png")
    temp_resized = temp_size.resize((370, 200), Image.ANTIALIAS)  #image size
    template = ImageTk.PhotoImage(temp_resized)
    template_image = Label(home, image=template, borderwidth="0")
    template_image.place(x="0", y="0")  # image position
    ...
    qutie = Button(home, image=quiteimg, borderwidth="0", activebackground='black',command=exitapp)
    qutie.place(x=120, y=100)

    home.mainloop()

def exitapp():
    response = messagebox.askyesno("Dodge the ball","Are you sure you want to quit? ")
...
background = (174, 232, 209)       #color
playerColor = (24, 158, 6)
...
def gameOver():
    mixer.music.stop()
    mixer.init()
    mixer.music.load('Images/gameover.mp3')
    mixer.music.play()
    loop = True
...
def displayScore():
    ...
    scoreText = font.render("Score: " + str(score), True, (222, 27, 27))
...
def gameLoop():
    ...
    while loop:
        ...
        if collide:
            ...
            mixer.music.pause()
            mixer.init()
            mixer.music.load('Images/increasescore.mp3')
            mixer.music.play()
            mixer.music.unpause()
            target.generateNewCoord()
